...
model_names = ["firstStage_BGShadowNet"]
...
def get_model(name: str, in_channels: int = True, pretrained: bool = True) -> nn.Module:
    ...
    if name not in model_names:
        message = (
            ...
            "You have to choose cbeNet/cam_benet(BENet), srnet(SR-Net, unimplemented), stcgan(ST-CGAN, unimplemented) as a model."
        )
    ...
    if name == "firstStage_BGShadowNet":
        firstStage_BGShadowNet = getattr(models, "firstStage_BGShadowNet")(pretrained=pretrained)
        discriminator = getattr(models, "discriminator")(pretrained=pretrained)
        model = [firstStage_BGShadowNet, discriminator]
    else:
